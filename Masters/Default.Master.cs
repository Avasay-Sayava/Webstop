...
<ul>
  {((Session["Signin"] as int? ?? 0) == 0
    ...
    : @"<li><a href='Signout'>Sign Out</a></li>")}
      ...
      AdminBtn = $"{SQL.Manager.ExecuteQuery($"select Type from Users where Id='{Session["Signin"] ?? 0}'")[0, 0]}";
      AdminBtn = $"{SQL.Manager.ExecuteQuery(SQL.Syntax.Select("Users", new string[]{"Type"}))[0, 0]}".Equals("255")
